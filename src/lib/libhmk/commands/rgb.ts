...
export async function getRgbConfig(
    ...
    while (data.length < configSize) {
        ...
        if (response.commandId !== HMK_Command.GET_RGB_CONFIG || !response.rgbConfigData) {
            throw new Error(`Failed to get RGB config for profile ${params.profile}`)
        }

        const chunkLen = Math.min(GET_SET_RGB_CONFIG_MAX_ENTRIES, configSize - data.length)
...
export async function setRgbConfig(
    ...
    // Send chunks
    for (let offset = 0; offset < rawBytes.length; offset += GET_SET_RGB_CONFIG_MAX_ENTRIES) {
        const chunk = rawBytes.slice(offset, offset + GET_SET_RGB_CONFIG_MAX_ENTRIES)
